...
func (r *SubscriberRunner) SendCloudEvent(recv *store.ReadResponse) error {
	...
	ctx = cloudevents.ContextWithRetriesExponentialBackoff(ctx, r.spec.Spec.Delivery.MaxRequestDuration.Duration, r.spec.Spec.Delivery.MaxRetries)
...
func getSubscriberName(name string, hostname string, s Subscriber) []byte {
	return []byte(fmt.Sprintf("%s/%s/%s", name, hostname, s.String()))
}

func NewSubscribeRunner(spec Subscription, s Subscriber, conn *grpc.ClientConn, client dynamic.Interface, streamName string) *SubscriberRunner {
